...
The study of set theory itself has led to many interesting results concerning the nature and philosophy of mathematics, as well as computer science.
Zermelo-Frankel set theory ($\mathit{ZF}$) was developed over the $20^{\mathit{th}}$ century in order to create a theory of sets free of paradoxes.
...
Ordered pairs are introduced as a structure distinct from sets, often used to in mathematics to define co-ordinate systems, as well as rigorous definitions functions and relations.
...
We hope that this will begin bridge the gap between a widely accepted foundation of mathematics, and standard mathematical practice. 
...
\begin{minipage}[c]{0.9\textwidth}
  ...
  Date: 22/11/2018
\end{minipage}
\footnotetext{This declaration is taken from Appendix C.1 in \cite{dissguide}}
\clearpage
...
The axiomatic method can be found as early as Euclid, and even in the contemporary mathematics of group theory.
The study of such systems has also been a prominent area of research in mathematical logic, and computer science.
...
The axioms of $\mathit{ZF}$ attempt to characterise the notion of set, by listing axioms in first-order logic in terms of the \emph{membership relation} $\in$.
These axioms allow us to formally prove the existence and properties of various sets.
For instance, the first axiom of $\mathit{ZF}$ is the axiom of extensionality. asserts that two sets are equal if and only if they contain the same elements, and the axiom schema of specification allows us to create subsets of some initial set, containing only elements which satisfy some common property.
...
These sets are defined in this way to adhere to the structure and truth of the natural numbers, for instance, they ordered by the memership relation, since $0\in 1$, $2\notin 1$, etc.
...
For example --- when using the Von Neumann definition of the ordinals --- the natural numbers $1,2,3$ are actually non-empty sets, defined as $\{0\}, \{0,1\}$,$\{0,1,2\}$, respectively.
...
If $\mathit{ZF}$ is assumed to be consistent, and an interpetation can exist, then we can conlude that our new system is also consistent.

It would also be ideal to introduce a so-called ``garbage" or ``exception" object, should be returned when we try to construct sets or pairs that do not exist, or from a function application on an element which is outside of the domain of the function.
...
The theory must have sufficient power to express the high-level mathematical concepts taught in undergraduate mathematics and computer science courses.
